...
VITE_OIDC_AUTHORITY=${VITE_OIDC_AUTHORITY:-http://localhost:8080/realms/whattech}
VITE_OIDC_CLIENT_ID=${VITE_OIDC_CLIENT_ID:-mr-platform-web}
...
# Keycloak (set the base URL the *server container* uses to reach Keycloak)
KEYCLOAK_BASE_URL=${KEYCLOAK_BASE_URL:-http://keycloak:8080}
KEYCLOAK_REALM=${KEYCLOAK_REALM:-whattech}
KEYCLOAK_WEB_CLIENT_ID=${KEYCLOAK_WEB_CLIENT_ID:-mr-platform-web}
KEYCLOAK_ADMIN_CLIENT_ID=${KEYCLOAK_ADMIN_CLIENT_ID:-mr-platform-admin}
KEYCLOAK_ADMIN_CLIENT_SECRET=${KEYCLOAK_ADMIN_CLIENT_SECRET:-mr-platform-admin-secret-change-me}
KEYCLOAK_PLATFORM_ADMIN_ROLE=${KEYCLOAK_PLATFORM_ADMIN_ROLE:-platform-admin}
KEYCLOAK_DISABLED=${KEYCLOAK_DISABLED:-false}

echo "Building images..."
echo "  - $FRONTEND_IMAGE"
docker build --network=host -t "$FRONTEND_IMAGE" -f "$ROOT/frontend/Dockerfile" "$ROOT/frontend" --build-arg VITE_TUSD_ENDPOINT="$VITE_TUSD_ENDPOINT" --build-arg VITE_TUSD_PATH_PREFIX="$VITE_TUSD_PATH_PREFIX" --build-arg VITE_OIDC_AUTHORITY="$VITE_OIDC_AUTHORITY" --build-arg VITE_OIDC_CLIENT_ID="$VITE_OIDC_CLIENT_ID"
echo "  - $SERVER_IMAGE"
...
docker run -d --name mysql --restart "$RESTART_POLICY" --network "$NETWORK" -p "$MYSQL_PORT:3306" -e MYSQL_ROOT_PASSWORD="$MYSQL_ROOT_PASSWORD" -e MYSQL_DATABASE="$MYSQL_DATABASE" -e MYSQL_USER="$MYSQL_USER" -e MYSQL_PASSWORD="$MYSQL_PASSWORD" -v "$MYSQL_DATA_MOUNT" "$MYSQL_IMAGE"
...
docker run -d --name server --restart "$RESTART_POLICY" --network "$NETWORK" -p 1337:1337 -e HOST=0.0.0.0 -e PORT=1337 -e DATABASE_CLIENT=mysql -e DATABASE_HOST=mysql -e DATABASE_PORT=3306 -e DATABASE_NAME="$MYSQL_DATABASE" -e DATABASE_USERNAME="$MYSQL_USER" -e DATABASE_PASSWORD="$MYSQL_PASSWORD" -e APP_KEYS="$APP_KEYS" -e API_TOKEN_SALT="$API_TOKEN_SALT" -e ADMIN_JWT_SECRET="$ADMIN_JWT_SECRET" -e TRANSFER_TOKEN_SALT="$TRANSFER_TOKEN_SALT" -e JWT_SECRET="$JWT_SECRET" -e ENCRYPTION_KEY="$ENCRYPTION_KEY" -e KEYCLOAK_BASE_URL="$KEYCLOAK_BASE_URL" -e KEYCLOAK_REALM="$KEYCLOAK_REALM" -e KEYCLOAK_WEB_CLIENT_ID="$KEYCLOAK_WEB_CLIENT_ID" -e KEYCLOAK_ADMIN_CLIENT_ID="$KEYCLOAK_ADMIN_CLIENT_ID" -e KEYCLOAK_ADMIN_CLIENT_SECRET="$KEYCLOAK_ADMIN_CLIENT_SECRET" -e KEYCLOAK_PLATFORM_ADMIN_ROLE="$KEYCLOAK_PLATFORM_ADMIN_ROLE" -e KEYCLOAK_DISABLED="$KEYCLOAK_DISABLED" -v "$STRAPI_UPLOADS_HOST_DIR:/app/public/uploads" -v /etc/machine-id:/etc/machine-id:ro "$SERVER_IMAGE"
